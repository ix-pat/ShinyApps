...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      numericInput("n0", "n per approssimazione normale", min = 0, max = 10000, value = 30, step = 10),
    ...
    mainPanel(
      plotOutput("plotCondiviso", height = "800px")
    )
  )
)
...
server <- function(input, output) {
  ...
  output$plotCondiviso <- renderPlot({
    ...
    ic95 <- function(nn, pi) {
      ref_y <- if (input$scala_relativa) input$pi_val else input$n0 * input$pi_val      
      
      se <- sqrt(nn * pi * (1 - pi))
      lo <- ref_y - 1.96 * if (input$scala_relativa) se / nn else se
      hi <- ref_y + 1.96 * if (input$scala_relativa) se / nn else se
      c(lo,ref_y, hi)
    }
    ...
    yat <- if (input$scala_relativa) (0:10)/10 else c(0,tick_seq(input$n)[-1])
    if (input$scala_relativa & input$n<=10) yat <- c(1,yat)
    axis(1, at = tick_seq(input$n))
    axis(2, yat, las = 2)
    abline(h=yat,lty=4,col="grey90")
    abline(v=tick_seq(input$n),lty=4,col="grey90")
    n_tracce <- length(tracce$lista)
    if (n_tracce>0){
    for (i in 1:n_tracce) {
      y <- tracce$lista[[i]]
      col_ <- ifelse(i == n_tracce,"darkred","grey")
      if (length(y) == input$n) {
        lines(1:input$n, y, col = col_, lwd = 1)
        points(1:input$n, y, col = col_, pch = 16, cex = 0.4)
        if (input$n <= 10) {
          txt <- paste0(round(y * if (input$scala_relativa) 100 else 1, 2), if (input$scala_relativa) "%" else "")
          text(1:input$n-.1, y, txt, pos = 3,col=col_, offset = .5)
        }
      }
    }
      if (input$mostra_normale){
        y <- tracce$lista[[n_tracce]][input$n0]
        segments(input$n0,y,input$n,y,col="red",lty=2)
      }
      
    }

    if (input$scala_relativa) {
      abline(h = input$pi_val, lty = 2)
      if (input$mostra_normale) {
        abline(v = input$n0, lty = 2,col="red",lwd=2)
        
        bounds <- ic95(input$n0, input$pi_val)
        # segments(input$n0, bounds, input$n, bounds, lty = 2,col="red")
      }
    } else {
      # Scala assoluta: medie e intervalli sono rette crescenti
      abline(a = 0, b = input$pi_val, lty = 2,col="grey40")
      if (input$mostra_normale) {
        abline(v = input$n0, lty = 2,col="red",lwd=2)
        
        bounds <- ic95(input$n0, input$pi_val)
        # if (input$n0 <= 10) segments(0,bounds[2],input$n0,bounds[2], lty = 2,col="red") else
        # segments(input$n0, bounds, input$n, bounds, lty = 2,col="red")
    ...
    if (input$mostra_normale && input$n >= 30) {
      ...
      n  <- input$n
      se <- sqrt(n0 * pi * (1 - pi))
      se <- ifelse(input$scala_relativa,se/n0,se)
      mu <- ifelse(input$scala_relativa,pi,pi*n0)
      ...
      arrows(0, mu - 1.96 * se, 0, mu + 1.96 * se, lty = 1,
             col="red",angle = 90,code = 3,length = .01,lwd=2)
      axis(2, yat,las = 2)
      axis(1)
      ...
      n_tracce <- length(tracce$lista)
      if (n_tracce>0){
        Y <- matrix(unlist(tracce$lista),nrow = input$n)
        col_ <- c(rep("grey",times=n_tracce-1),"darkred")
        points(rep(0,times=n_tracce),Y[input$n0,],col=col_,pch=4)
      }
