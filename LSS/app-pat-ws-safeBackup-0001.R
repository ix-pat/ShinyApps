...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      numericInput("beta0", "Intercetta (beta0):", value = 0,step = .1),
      numericInput("beta1", "Pendenza (beta1):", value = 1,step = .1)
    ),
    
    mainPanel(
      plotOutput("regressionPlot"),
      h3(textOutput("mseText"), align = "center")
    )
  )
)
...
server <- function(input, output) {
  output$regressionPlot <- renderPlot({
    ...
    ggplot(data, aes(x = x, y = y)) +
      geom_point(size = 3) +
      geom_segment(aes(x = x, xend = x, y = y, yend = y_pred), color = "gray", linetype = "dotted") +
