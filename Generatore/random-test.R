...
gen_test <- function(files){
  #files <- c("2021.Rmd","2022.Rmd","2023.Rmd","2024.Rmd")
  ...
  if (sum(punti_tot[[es5[1]]][[es5[2]]]$e5)<=14) {N_eserc <- 6  } else {N_eserc <- 5}
  ...
  html <- F
  rmarkdown::render(
    input = "compito_sol.Rmd",
    output_format = "bookdown::pdf_document2",
    output_file = "www/compito.pdf"
  )
  html <- T
  rmarkdown::render(
    "compito_sol.Rmd",
